...
query1="%20schema%3Aabout%20%3Fo%20.%0A%20%20%3Fo%20rdfs%3Alabel%20%3Flabel%20filter(lang(%3Flabel)%20%3D%20%22en%22)%20.%0A%20%20OPTIONAL%20%7B%3Fo%20schema%3Adescription%20%3Fdesc%20filter(lang(%3Fdesc)%20%3D%20%22en%22)%7D%0A%7D%20%20 -X GET";
query3="$query$term$query1";
query3=${query3// /%20};
echo "$query3";
SPARQL_RESULTS="$(curl "$query3")";
echo "<term id='$term'>$SPARQL_RESULTS</term>" >> "results_english_sitelink.xml"
